...
def deploy(
    ...
    thunder_bootstrap = k8s.core.v1.ConfigMap(
        ...
        opts=pulumi.ResourceOptions(provider=k8s_provider, depends_on=[thunder_ns]),
    )
    ...
    thunder = k8s.helm.v3.Release(
        ...
        opts=pulumi.ResourceOptions(
            ...
            depends_on=[thunder_bootstrap],
            custom_timeouts=pulumi.CustomTimeouts(create=f"{TIMEOUT_DEFAULT}s"),
        ),
    )

    wait_thunder = sleep("thunder", SLEEP_AFTER_THUNDER, opts=pulumi.ResourceOptions(depends_on=[thunder]))
                    ...
                    containers=[
                        k8s.core.v1.ContainerArgs(
                            ...
                            security_context=k8s.core.v1.SecurityContextArgs(
                                ...
                                read_only_root_filesystem=thunder_security_context.get("readOnlyRootFilesystem", True),
    ...
    backstage_es = k8s.apiextensions.CustomResource(
        ...
        opts=pulumi.ResourceOptions(provider=k8s_provider, depends_on=[wait_thunder]),
    )

    wait_eso_sync = sleep(
        "eso-sync",
        SLEEP_AFTER_ESO_SYNC,
        opts=pulumi.ResourceOptions(depends_on=[backstage_es, thunder_setup_rerun]),
    )
    ...
    label_ns = LabelNamespace(
        ...
        opts=pulumi.ResourceOptions(depends_on=[cp_chart, patch_workflows]),
    )

    return ControlPlaneResult(helm_chart=cp_chart, label_ns=label_ns)
